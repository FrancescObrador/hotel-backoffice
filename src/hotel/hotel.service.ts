...
export class HotelService {
  ...
  async findAll(pagination: PaginationDto): Promise<Hotel[]> {
    return await this.hotelRepo.find({skip: pagination.skip, take: pagination.limit})
  }
  ...
  async addFeature(addHotelFeatureDto: AddHotelFeatureDto){
  ...
  async removeMedia(hotelId: number, mediaId: number){
    const media = await this.hotelMediaRepo.findOneBy({ id: mediaId, hotel: { id: hotelId } })
    if (!media) {
